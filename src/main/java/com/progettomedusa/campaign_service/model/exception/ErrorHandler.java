...
import com.progettomedusa.campaign_service.controller.CampaignController;
...
public class ErrorHandler implements RequestBodyAdvice {
    ...
    private static void setErrorHandler(ErrorHandler errorHandler) { requestInfoThreadLocal.set(errorHandler); }

    @ExceptionHandler(Exception.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public final CommonErrorResponse handleAllExceptions(Exception ex, HttpServletRequest request) {
        log.error("Exception: ", ex);
        return buildError(request, ErrorMsg.CPGSRV99.getCode(), ErrorMsg.CPGSRV99.getMessage(), ex.getMessage(), DomainMsg.CAMPAIGN_SERVICE_TECHNICAL.getName());
    }

    @ExceptionHandler({ RuntimeException.class })
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public final CommonErrorResponse handleRuntimeExceptions(RuntimeException ex, HttpServletRequest request) {
        log.error("Exception: ", ex);
        return buildError(request, ErrorMsg.CPGSRV99.getCode(), ErrorMsg.CPGSRV99.getMessage(), ex.getMessage(), DomainMsg.CAMPAIGN_SERVICE_TECHNICAL.getName());
    }
    ...
    public final CommonErrorResponse handleValidationException(Exception ex, HttpServletRequest request) {
        ...
        if (ex instanceof MethodArgumentNotValidException methodArgumentNotValidException) {
            ...
            return buildError(request, ErrorMsg.CPGSRV01.getCode(), ErrorMsg.CPGSRV01.getMessage(), ex.getMessage(), DomainMsg.MICROSERVICE_FUNCTIONAL.getName());
    ...
    private CommonErrorResponse buildInvalidFieldsError(HttpServletRequest request, Map<String, String> fieldsMap) {
        ...
        return buildError(request, ErrorMsg.CPGSRV01.getCode(), message, StringUtils.join(fieldsMap.values(), ','), DomainMsg.MICROSERVICE_FUNCTIONAL.getName());
    }
